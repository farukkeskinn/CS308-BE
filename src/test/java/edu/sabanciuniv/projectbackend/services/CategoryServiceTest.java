...
class CategoryServiceTest {
    ...
    //ADDED AFTER PROGRESS DEMO (NEW UNIT TESTS)
    @Test
    void deleteCategory_withMultipleProducts_reassignsAllProducts() {
        Category sub = new Category(); sub.setCategoryId(2);
        Category parent = new Category(); parent.setCategoryId(1);
        sub.setParentCategory(parent);

        Product p1 = new Product(); p1.setCategory(sub);
        Product p2 = new Product(); p2.setCategory(sub);
        Product p3 = new Product(); p3.setCategory(sub);

        when(categoryRepository.findById(2)).thenReturn(Optional.of(sub));
        when(productRepository.findByCategory_CategoryId(2)).thenReturn(List.of(p1, p2, p3));
        ...
        verify(categoryRepository).delete(sub);
        assertEquals(parent, p1.getCategory());
        assertEquals(parent, p2.getCategory());
        assertEquals(parent, p3.getCategory());
    }

    //ADDED AFTER PROGRESS DEMO (NEW UNIT TESTS)
    @Test
    void saveCategory_withExistingCategory_updatesCategory() {
